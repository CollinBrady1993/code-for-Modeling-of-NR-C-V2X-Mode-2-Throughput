...
fc = 5.89;%GHz
Pt = 23;%dBm
PtWpHzPSCCH = 10^((Pt-30)/10)/(180000*10);%power spectral density in one PSCCH RB as calculated by ns-3
rbPerSubchannel = 50;
...
C1 = tau*(T2-T1+1)*(1/(1000*2^mu))*rhoUe;
C2 = tau*(Nsh/(1000*2^mu))*rhoUe*(numLanes);
...
for i = 1:length(d)
    ...
    Nrxt(i) = trapz(dtemp,C2*(1-deltaFTRM).*(deltaFTRMS));
    NrxB(i) = trapz(dtemp,C2*(1-deltaFTRM).*(1-deltaFTRMS));
